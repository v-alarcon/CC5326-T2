...
//init config
char* config_transport [2];
//buffer for protocol 0 of 13 bytes
uint8_t protocol0[13];

//buffer for protocol 1 of 17 bytes
uint8_t protocol1[17];

//buffer for protocol 2 of 27 bytes
uint8_t protocol2[27];

//buffer for protocol 3 of 55 bytes
uint8_t protocol3[55];
...
char* header(char protocol, char transportLayer){
	...
	// 6 bytes para MAC Adress bluetooth
    char* mac_str = "3c:61:05:65:9d:ba";
    //transform the str into a mac in bytes and copy into mac
    uint8_t mac[6];
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        if(config_transport[0] == '0' && config_transport[1] == '0'){
            ESP_LOGI(GATTS_TAG, "Reading Protocol 0");
            rsp.attr_value.len = 13;
            // copy the protocol0 into the rsp
            memcpy(rsp.attr_value.value, protocol0, 13);
        }
        else if (config_transport[0] == '1' && config_transport[1] == '0')
        {
            ESP_LOGI(GATTS_TAG, "Reading Protocol 1");
            rsp.attr_value.len = 17;
            // copy the protocol1 into the rsp
            memcpy(rsp.attr_value.value, protocol1, 17);
        }
        else if (config_transport[0] == '2' && config_transport[1] == '0')
        {
            ESP_LOGI(GATTS_TAG, "Reading Protocol 2");
            rsp.attr_value.len = 27;
            // copy the protocol2 into the rsp
            memcpy(rsp.attr_value.value, protocol2, 27);
        }
        else if (config_transport[0] == '3' && config_transport[1] == '0')
        {
            ESP_LOGI(GATTS_TAG, "Reading Protocol 3");
            rsp.attr_value.len = 55;
            // copy the protocol3 into the rsp
            memcpy(rsp.attr_value.value, protocol3, 55);
        }
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            // copy the value into str
            char* str = malloc(param->write.len+1);
            memcpy(str, param->write.value, param->write.len);
            // add a 0 at the end
            str[param->write.len] = 0;
            // print the value
            ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value: %s", str);

            // protocol 0 
            if (str[0]=='0' && str[1]=='0')
            {
                ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, protocol 0 continue");
                // set the conf variable
                config_transport[0] = str[0];
                config_transport[1] = str[1];
                // create the header
                char* head = header('0', '0');
                // copy the header into the protocol0
                memcpy(protocol0, head, 12);
                // create battery level
                int batteryLevel = getBatteryLevel();
                // copy the battery level into the protocol0
                memcpy(protocol0+12, &batteryLevel, 1);
                ESP_LOGI(GATTS_TAG, "Protocol 0: %d", batteryLevel);
            }
            else if (str[0]=='1' && str[1]=='0')
            {
                ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, protocol 1 continue");
                // set the conf variable
                config_transport[0] = str[0];
                config_transport[1] = str[1];
                // create the header
                char* head = header('1', '0');
                // copy the header into the protocol1
                memcpy(protocol1, head, 12);
                // create the battery level
                int batteryLevel = getBatteryLevel();
                // copy the battery level into the protocol1
                memcpy(protocol1+12, &batteryLevel, 1);
                // create the timestamp
                time_t now;
                time(&now);
                // copy the timestamp into the protocol1
                memcpy(protocol1+13, &now, 4);
                ESP_LOGI(GATTS_TAG, "Protocol 1");
            }
            else if (str[0]=='2' && str[1]=='0')
            {
                ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, protocol 2 continue");
                // set the conf variable
                config_transport[0] = str[0];
                config_transport[1] = str[1];
                // create the header
                ...
                // copy the header into the protocol1
                memcpy(protocol2, head, 12);
                // create the battery level
                int batteryLevel = getBatteryLevel();
                // copy the battery level into the protocol1
                memcpy(protocol2+12, &batteryLevel, 1);
                // create the timestamp
                time_t now;
                time(&now);
                // copy the timestamp into the protocol1
                memcpy(protocol2+13, &now, 4);
                // create the data
                char* data = get_thpc();
                // copy the data into the protocol2
                memcpy(protocol2+17, data, 10);

                ESP_LOGI(GATTS_TAG, "Protocol 2");
            }
            else if (str[0]=='3' && str[1]=='0')
            {
                ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, protocol 3 continue");
                // set the conf variable
                config_transport[0] = str[0];
                config_transport[1] = str[1];
                // create the header
                char* head = header('3', '0');
                // copy the header into the protocol3
                memcpy(protocol3, head, 12);
                // create the battery level
                int batteryLevel = getBatteryLevel();
                // copy the battery level into the protocol3
                memcpy(protocol3+12, &batteryLevel, 1);
                // create the timestamp
                time_t now;
                time(&now);
                // copy the timestamp into the protocol3
                memcpy(protocol3+13, &now, 4);
                // create the data
                char* data = get_thpc();
                // copy the data into the protocol3
                memcpy(protocol3+17, data, 10);
                // create the data
                data = get_kpi();
                // copy the data into the protocol3
                memcpy(protocol3+27, data, 28);      

                ESP_LOGI(GATTS_TAG, "Protocol 3");
            }
